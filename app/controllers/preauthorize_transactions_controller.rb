...
class PreauthorizeTransactionsController < ApplicationController
  ...
  def initiate
    validation_result = TransactionService::Validation::NewTransactionParams.validate(params).and_then { |params_entity|
  ...
  def handle_tx_response(tx_response, gateway)
    if !tx_response[:success]
      render_error_response(request.xhr?, t("error_messages.#{gateway}.generic_error")+tx_response.inspect, action: :initiate)

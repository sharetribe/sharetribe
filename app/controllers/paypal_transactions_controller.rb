class PaypalTransactionsController < ApplicationController
  ...
  def paypal_checkout_order_success
    if params[:token].present?
      listing_author_id = Transaction.find(params[:transaction_id]).author.id
      ...
      MarketplaceService::Transaction::Command.transition_to(params[:transaction_id], "preauthorized")
      return redirect_to person_transaction_path(:person_id => @current_user.id, :id => params[:transaction_id])

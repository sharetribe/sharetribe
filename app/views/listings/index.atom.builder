atom_feed :language => 'en-US', 'xmlns:georss' => 'http://www.georss.org/georss', 'xmlns:st'  => 'http://www.sharetribe.com/SharetribeFeed' do |feed|
  ...
  listings.each do |listing|
    feed.entry( listing, url: listing_url(listing, host: @current_community.full_domain(port: ''))) do |entry|
      ...
      entry.st :listing_type, :term => direction_map[listing.transaction_type_id], :label => localized_listing_type_label(direction_map[listing.transaction_type_id])
      ...
      entry.st :share_type, :term => listing.transaction_type_id, :label => localized_transaction_type_label(listing.transaction_type).capitalize if listing.transaction_type

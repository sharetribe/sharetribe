module LocationsHelper
  ...
  def get_coordinates(place_name)
    ...
    #puts "Sending request to Google maps api: http://maps.googleapis.com/maps/api/geocode/json?address=#{escaped_place_name}&region=fi&sensor=false"
    response = JSON.parse(RestClient.get("http://maps.googleapis.com/maps/api/geocode/json?address=#{escaped_place_name}&region=fi&sensor=false"))

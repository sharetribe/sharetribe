module CategoriesHelper
  ...
  def self.load_categories_and_share_types_to_db(community, categories, share_types, translations={})
    community_id = community ? community.id : nil
    ...
    Kassi::Application.config.AVAILABLE_LOCALES.each do |loc|
      ...
      Category.find_each do |category|
        begin 
          translated_string =  (translations[locale] && translations[locale][category.name]) || I18n.t!(category.name, :locale => locale, :scope => ["common", "categories"], :raise => true)
      ...
      ShareType.find_each do |share_type|
        begin
          translated_string = (translations[locale] && translations[locale][share_type.name]) || I18n.t!(share_type.name, :locale => locale, :scope => ["common", "share_types"], :raise => true)
    ...
    Category.all.each do |category|
      sell_category = CommunityCategory.where("category_id = ? AND share_type_id = ? AND community_id = ?", category.id.to_s, sell.id.to_s, community_id.to_s).first
      sell_category.update_attributes(:price => true, :price_quantity_placeholder => "mass", :payment => true) if sell_category  

      buy_category = CommunityCategory.where("category_id = ? AND share_type_id = ? AND community_id = ?", category.id.to_s, buy.id.to_s, community_id.to_s).first
      buy_category.update_attributes(:payment => true) if buy_category

    end
    
    
    #TODO: Add here a custom line to put , :price_quantity_placeholder => "long_time" for renting out spaces WHEN BUILDING DEFAULT CATEGORIES

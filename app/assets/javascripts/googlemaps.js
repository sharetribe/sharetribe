...
$.validator.addMethod("address_validator", function(value, element, param) {
  var check = null;
  ...
  var elem_prefix = "";
  if (pref[0].match("person")) elem_prefix = "person";
  else if (pref[0].match("community")) elem_prefix = "community";
  else elem_prefix = pref[0] + "_" + pref[1];

  var emptyfield =
    $('input[id$="latitude"][id^=' + elem_prefix + "]").attr("value") || "";

  if (emptyfield != "") check = true;
  else check = false;
...
function update_map(field) {
  if (geocoder) {
    geocoder.geocode({ address: field.value.replace("&", "") }, function(
      response,
      info
    ) {
...
function startRoute(latitude, longitude) {
  ...
  document.getElementById(
    "listing_destination_loc_attributes_address"
  ).value = bar;
...
function wrongLocationRoute(field) {
  ...
  document.getElementById(
    field + "_loc_attributes_google_address"
  ).value = null;
...
function updateEditTextBoxes() {
  ...
  document.getElementById(
    "listing_origin_loc_attributes_google_address"
  ).value = foo;
  document.getElementById(
    "listing_destination_loc_attributes_google_address"
  ).value = bar;
  document.getElementById(
    "listing_origin_loc_attributes_latitude"
  ).value = directionsDisplay
    .getDirections()
    .routes[0].legs[0].start_location.lat();
  document.getElementById(
    "listing_origin_loc_attributes_longitude"
  ).value = directionsDisplay
    .getDirections()
    .routes[0].legs[0].start_location.lng();
  document.getElementById(
    "listing_destination_loc_attributes_latitude"
  ).value = directionsDisplay
    .getDirections()
    .routes[0].legs[0].end_location.lat();
  document.getElementById(
    "listing_destination_loc_attributes_longitude"
  ).value = directionsDisplay
    .getDirections()
          ...
          google.maps.event.addListener(marker, "click", function() {
            ...
            if (showingMarker == marker.getTitle()) {
              ...
              infowindow.setContent(
                "<div id='map_bubble'><div style='text-align: center; width: 360px; height: 70px; padding-top: 25px;'><img src='https://s3.amazonaws.com/sharetribe/assets/ajax-loader-grey.gif'></div></div>"
              );
              infowindow.open(map, marker);
...
function initialize_listing_map(
  listings,
  community_location_lat,
  community_location_lon,
  viewport,
...
function addListingMarkers(listings, viewport) {
      ...
      if (entry["latitude"]) {
        ...
        google.maps.event.addListener(marker, "click", function() {
          ...
          if (showingMarker == marker.getTitle()) {
            ...
            showingMarker = marker.getTitle();
            infowindow.setContent(
              "<div id='map_bubble'><img class='bubble-loader-gif' src='https://s3.amazonaws.com/sharetribe/assets/ajax-loader-grey.gif'></div>"
            );
  ...
  var latitudes = _(listings)
    .pluck("latitude")
    .filter()
    .map(Number)
    .value();
  var longitudes = _(listings)
    .pluck("longitude")
    .filter()
    .map(Number)
    .value();
  ...
  if (viewport && viewport.boundingbox) {
    ...
    var listingsBounds =
      latitudes.length && longitudes.length
        ? {
            sw: [_.min(latitudes), _.min(longitudes)],
            ne: [_.max(latitudes), _.max(longitudes)]
          }
        : null;

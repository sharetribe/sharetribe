...
(function(module) {
  ...
  var BANK_RULES = {
    AU: {
      account_number: {
        format: "format_varies_by_bank",
        regexp: "[0-9]{6,10}",
        test_regexp: "[0-9]{6,10}"
      },
      routing_number: {
        title: "bsb",
        format: "123456",
    ...
    AT: {
      account_number: {
        title: "IBAN",
        format: "AT611904300235473201",
        regexp: "AT[0-9]{2}[0-9]{16}",
        test_regexp: "AT" + TEST_IBAN
      }
    },
    BE: {
      account_number: {
        title: "IBAN",
        format: "BE12345678912345",
        regexp: "BE[0-9]{2}[0-9]{12}",
        test_regexp: "BE" + TEST_IBAN
      }
    },
    BR: {
    ...
    DK: {
      account_number: {
        title: "IBAN",
        format: "DK5000400440116243",
        regexp: "DK[0-9]{2}[0-9]{14}",
        test_regexp: "DK" + TEST_IBAN
      }
    },
    FI: {
      account_number: {
        title: "IBAN",
        format: "FI2112345600000785",
        regexp: "FI[0-9]{2}[0-9]{14}",
        test_regexp: "FI" + TEST_IBAN
      }
    },
    FR: {
      account_number: {
        title: "IBAN",
        format: "FR1420041010050500013M02606",
        regexp: "FR[0-9]{2}[0-9]{10}[A-Z0-9]{11}[0-9]{2}",
        test_regexp: "FR" + TEST_IBAN
      }
    },
    DE: {
      account_number: {
        title: "IBAN",
        format: "DE89370400440532013000",
        regexp: "DE[0-9]{2}[0-9]{18}",
        test_regexp: "DE" + TEST_IBAN
      }
    },
    GI: {
      account_number: {
        title: "IBAN",
        format: "GI75NWBK000000007099453",
        regexp: "GI[0-9]{2}[A-Z]{4}[A-Z0-9]{15}",
        test_regexp: "GI" + TEST_IBAN
      }
    },
    HK: {
      account_number: {
        format: "123456-789",
        regexp: "[0-9]{5,6}-[0-9]{1,3}",
        test_regexp: "[0-9]{5,6}-[0-9]{1,3}"
      },
      routing_1: {
        title: "clearing_code",
        format: "123",
    ...
    IE: {
      account_number: {
        title: "IBAN",
        format: "IE29AIBK93115212345678",
        regexp: "IE[0-9]{2}[A-Z0-9]{4}[0-9]{14}",
        test_regexp: "IE" + TEST_IBAN
      }
    },
    IT: {
      account_number: {
        title: "IBAN",
        format: "IT60X0542811101000000123456",
        regexp: "IT[0-9]{2}[A-Z]{1}[0-9]{5}[0-9]{5}[A-Z0-9]{12}",
        test_regexp: "IT" + TEST_IBAN
      }
    },
    JP: {
      account_number: {
    ...
    LU: {
      account_number: {
        title: "IBAN",
        format: "LU280019400644750000",
        regexp: "LU[0-9]{2}[0-9]{3}[A-Z0-9]{13}",
        test_regexp: "LU" + TEST_IBAN
      }
    },
    MX: {
      account_number: {
    ...
    NL: {
      account_number: {
        title: "IBAN",
        format: "NL39RABO0300065264",
        regexp: "NL[0-9]{2}[A-Z]{4}[0-9]{10}",
        test_regexp: "NL" + TEST_IBAN
      }
    },
    NZ: {
      account_number: {
        format: "11-0000-0000000-010",
        regexp: "[0-9]{2}-[0-9]{4}-[0-9]{7}-[0-9]{2,3}",
        test_regexp: "[0-9]{2}-[0-9]{4}-[0-9]{7}-[0-9]{2,3}"
      }
    },
    NO: {
      account_number: {
        ...
        regexp: "NO[0-9]{2}[0-9]{11}",
        test_regexp: "NO" + TEST_IBAN
      }
    },
    PT: {
      account_number: {
        title: "IBAN",
        format: "PT50123443211234567890172",
        regexp: "PT[0-9]{2}[0-9]{4}[0-9]{4}[0-9]{11}[0-9]{2}",
        test_regexp: "PT" + TEST_IBAN
      }
    },
    SG: {
      account_number: {
    ...
    ES: {
      account_number: {
        title: "IBAN",
        format: "ES9121000418450200051332",
        regexp: "ES[0-9]{2}[0-9]{20}",
        test_regexp: "ES" + TEST_IBAN
      }
    },
    SE: {
      account_number: {
        title: "IBAN",
        format: "SE3550000000054910000003",
        regexp: "SE[0-9]{2}[0-9]{20}",
        test_regexp: "SE" + TEST_IBAN
      }
    },
    CH: {
      account_number: {
        title: "IBAN",
        format: "CH9300762011623852957",
        regexp: "CH[0-9]{2}[0-9]{5}[A-Z0-9]{12}",
        test_regexp: "CH" + TEST_IBAN
      }
    },
    GB: {
      account_number: {
  ...
  function update_bank_number_form(country) {
    ...
    $(".bank-account-number label:first").text(
      i18n_label(
        rule_account_number["title"],
        i18n_label("account_number", "Account number")
      ) + "*"
    );
    $(".bank-routing-number label:first").text(
      i18n_label(
        rule_routing_number["title"],
        i18n_label("routing_number", "Routing number")
      ) + "*"
    );
    $(".bank-routing-1 label:first").text(
      i18n_label(
        rule_routing_1["title"],
        i18n_label("routing_1", "Bank code")
      ) + "*"
    );
    $(".bank-routing-2 label:first").text(
      i18n_label(
        rule_routing_2["title"],
        i18n_label("routing_2", "Branch code")
      ) + "*"
    );

    $(".bank-account-number input").attr(
      "placeholder",
      i18n_label(rule_account_number["format"], rule_account_number["format"])
    );
    $(".bank-routing-number input").attr(
      "placeholder",
      rule_routing_number["format"]
    );
  ...
  function explain_regexp(value) {
    ...
    t = t.replace(
      /\[0-9\]\{(\d+)}/g,
      ";$1 " + i18n_label("digits", "digits") + ";"
    );
    t = t.replace(
      /\[A-Z0-9\]\{(\d+)}/g,
      ";$1 " + i18n_label("digits_or_chars", "digits or chars") + ";"
    );
    t = t.replace(
      /\[0-9\]\{(\d+),(\d+)}/g,
      ";$1-$2 " + i18n_label("digits", "digits") + ";"
    );
    t = t.replace(
      /\[A-Z0-9\]\{(\d+),(\d+)}/g,
      ";$1-$2 " + i18n_label("digits_or_chars", "digits or chars") + ";"
    );
  ...
  $.validator.addMethod(
    ...
    function(field, element) {
      ...
      var def_title =
        field == "account_number" ? i18n_label(field, "Account number") : field;
      return (
        i18n_label(title, def_title) +
        " " +
        i18n_label("must_match", "must be in the following format:") +
        " " +
        explain_regexp(regexp)
      );
    }
  );
  // Canada
  $.validator.addMethod("ca-social-insurance-number", function(
    value,
    element,
    field
  ) {
              ...
              switch ((_context.prev = _context.next)) {
                case 0:
                  ...
                  person = {
                    ...
                    personal_id_number: ["US", "CA", "HK", "SG", "PR"].includes(
                      country
                    )

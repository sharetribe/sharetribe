...
(function(module) {
  ...
  function update_listing_form_view(
    locale,
    attribute_array,
    listing_form_menu_titles,
    ordered_attributes,
    selected_attributes
  ) {
    // Hide everything
    ...
    $(".selected-group").each(function() {
      if (selected_attributes[$(this).attr("name")] != null) {
        $(this)
          .find(
            "a.selected[data-id=" +
              selected_attributes[$(this).attr("name")] +
              "]"
          )
          .removeClass("hidden");
      }
  ...
  function get_listing_shapes_for_subcategory(
    category_id,
    subcategory_id,
    category_array
  ) {
    var category = find_by_id(Number(category_id), category_array);
    var subcategory = find_by_id(
  ...
  function should_show_menu_for(
    ...
    if (attribute_selected(attribute, selected_attributes)) {
      ...
      if (
        ...
        var subcategories = get_subcategories_for(
      ...
      if (
        ...
        if (attribute_selected("subcategory", selected_attributes)) {
          listing_shapes = get_listing_shapes_for_subcategory(
            selected_attributes["category"],
            selected_attributes["subcategory"],
            attribute_array
          );
        } else {
          listing_shapes = get_listing_shapes_for_category(
            selected_attributes["category"],
            attribute_array
          );
        }
  ...
  function display_option_group(
    group_type,
    selected_attributes,
    attribute_array
  ) {
    $(".option-group[name=" + group_type + "]")
      .children()
      .each(function() {
  ...
  function select_listing_form_menu_link(
    link,
    locale,
    attribute_array,
    listing_form_menu_titles,
    ...
    var shouldLoadForm = update_listing_form_view(
      locale,
      attribute_array,
      listing_form_menu_titles,
      ordered_attributes,
  ...
  var setPushState = function(selectedAttributes) {
    ...
    window.history.pushState(
      selectedAttributes,
      null,
      addQueryParams(url, selectedAttributes)
    );
  };
  ...
  var selectedAttributesFromQueryParams = function(search) {
    ...
    var attrsFromQuery = _.zipObject(
      without_q.split("&").map(function(keyValuePair) {
  ...
  module.initialize_new_listing_form_selectors = function(
    locale,
    attribute_array,
    listing_form_menu_titles
  ) {
    var ordered_attributes = ["category", "subcategory", "listing_shape"];
    var selected_attributes = selectedAttributesFromQueryParams(
    ...
    window.addEventListener("popstate", function(evt) {
      ...
      var shouldLoadForm = select_listing_form_menu_link(
        $(this),
        locale,
        attribute_array,
        listing_form_menu_titles,
  ...
  module.initialize_edit_listing_form_selectors = function(
    locale,
    attribute_array,
    listing_form_menu_titles,
    category,
    ...
    // Reset the view to initial state
    var shouldShowForm = update_listing_form_view(
      locale,
      attribute_array,
      listing_form_menu_titles,
      ordered_attributes,
  ...
  module.initialize_new_listing_form = function(
    ...
    $("#help_valid_until_link").click(function() {
      $("#help_valid_until").lightbox_me({ centered: true, zIndex: 1000000 });
    });
    ...
    var form_id =
      listing_id == "false" ? "#new_listing" : "#edit_listing_" + listing_id;
    ...
    module.listingForm = $(form_id).validate({
      errorPlacement: function(error, element) {
        if (element.attr("name") == "listing[valid_until(1i)]") {
          ...
          element.attr("name") == "listing[shipping_price_additional]"
        ) {
      ...
      rules: _.extend(numericRules, {
        ...
        "listing[price]": {
          required: pr,
          money: true,
          minimum_price_required: [minimum_price, subunit_to_unit]
        },
      ...
      messages: {
        "listing[valid_until(1i)]": {
          min_date: date_message,
          max_date: date_message
        },
    ...
    var status = window.ST.imageUploader(listingImages, listingImageOpts).log(
      "status returned"
    );
    ...
    var isLoading = status.map(function(stats) {
      return stats.loading > 0;
    });

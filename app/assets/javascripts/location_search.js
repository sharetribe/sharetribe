...
(function(module) {
  module.initializeLocationSearch = function initializeLocationSearch(
    selectors
  ) {
    ...
    var autocomplete = new window.google.maps.places.Autocomplete(searchInput, {
      bounds: { north: -90, east: -180, south: 90, west: 180 }
    });
    ...
    function updateViewportData(viewport) {
      if (viewport) {
        var boundingboxRadius = computeScale(
    ...
    function isValidPrintableKey(keycode) {
      return (
        keycode == 32 ||
        keycode == 13 ||
        keycode == 8 || // spacebar & return key & backspace
      ...
      ); // [\]' (in order) in standard US layout
    }

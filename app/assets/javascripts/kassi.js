...
function add_validator_methods() {
  ...
  */
  ...
  $.validator.addMethod("valid_username", function(value, element, param) {
  ...
  $.validator.addMethod("email_list", function(value, element, param) {
  ...
  $.validator.addMethod("min_date", function(value, element, is_rideshare) {
  ...
  $.validator.addMethod("max_date", function(value, element) {
    ...
    var maximum_date = new Date(
      new Date(current_time).setMonth(current_time.getMonth() + 6)
    );
    return get_date_from_date_select() < maximum_date;
  });

  $.validator.addMethod("positive_integer", function(value, element, param) {
    var n = ~~Number(value);
    return String(n) === value && n >= 0;
  });

  $.validator.addMethod("minimum_price_required", function(
    value,
    element,
    params
  ) {
    ...
    if (minimum_price == "") {
      ...
      return (
        minimum_price <=
        ST.paymentMath.parseSubunitFloatFromFieldValue(value, subunit_to_unit)
      );
    }
  });

  $.validator.addMethod("money", function(value, element, minimum_price) {
    var regex = /^\d*((\.|\,)\d{1,2})?$/;
    return value && regex.test(value);
  });

  $.validator.addMethod("max_bound", function(value, element, otherName) {
    var $otherInput = ST.utils.findElementByName(otherName);
    return Number(toNumber(value)) > numberVal($otherInput);
  });

  $.validator.addMethod("min_bound", function(value, element, otherName) {
    var $otherInput = ST.utils.findElementByName(otherName);
    return Number(toNumber(value)) < numberVal($otherInput);
  });
  $.validator.addMethod("number_conditional_decimals", function(
    value,
    element,
    decimalCheckbox
  ) {
  ...
  $.validator.addMethod("number_no_decimals", function(value, element, opts) {
    var numberRegex = /^\d+?$/;
    return value.length === 0 ? true : numberRegex.test(value);
  });

  $.validator.addMethod("number_decimals", function(value, element) {
    var decimalRegex = /^\d+((\.|\,)\d+)?$/;
    return value.length === 0 ? true : decimalRegex.test(value);
  });

  $.validator.addMethod("number_min", function(value, element, min) {
    return value.length === 0 ? true : toNumber(value) >= min;
  });

  $.validator.addMethod("number_max", function(value, element, max) {
    return value.length === 0 ? true : toNumber(value) <= max;
  });

  $.validator.addMethod("email_remove_spaces", function(value, element) {
    value = value.trim();
    $(element).val(value);
...
function initialize_send_person_message_form(locale) {
  ...
  $(form_id).validate({
    rules: {
      "conversation[message_attributes][content]": {
        required: true,
        minlength: 1
      }
    },
...
function initialize_reply_form(locale) {
  ...
  $("textarea").focus();
  prepare_ajax_form("#new_message", locale, {
    "message[content]": { required: true, minlength: 1 }
  });
}

function initialize_listing_view(locale) {
  $("#listing-image-link").click(function() {
    $("#listing-image-lightbox").lightbox_me({
      centered: true,
      zIndex: 1000000
    });
  });
  auto_resize_text_areas("listing_comment_content_text_area");
  prepare_ajax_form("#new_comment", locale, {
...
function initialize_give_feedback_form(
  locale,
  grade_error_message,
  text_error_message
) {
...
function initialize_signup_form(
  locale,
  username_in_use_message,
  invalid_username_message,
  email_in_use_message,
  invalid_invitation_code_message,
  name_required,
  invitation_required
) {
  ...
  $(form_id).validate({
    ...
    rules: {
      "person[username]": {
        ...
        maxlength: 20,
        valid_username: true,
        remote: "/people/check_username_availability"
      },
      "person[given_name]": { required: name_required, maxlength: 30 },
      "person[family_name]": { required: name_required, maxlength: 30 },
      "person[email]": {
        required: true,
        email_remove_spaces: true,
        remote: "/people/check_email_availability_and_validity"
      },
      ...
      "person[password2]": {
        required: true,
        minlength: 4,
        equalTo: "#person_password1"
      },
      invitation_code: {
        required: invitation_required,
        remote: "/people/check_invitation_code"
      }
    },
    messages: {
      "person[username]": {
        valid_username: invalid_username_message,
        remote: username_in_use_message
      },
      "person[email]": {
        remote: email_in_use_message,
        email_remove_spaces: $.validator.messages.email
      },
...
function initialize_update_account_info_form(
  locale,
  change_text,
  cancel_text,
  email_in_use_message,
  one_email_must_receive_notifications_message
) {
  $(".account-new-email-link").click(function(event) {
  ...
  $(email_form_id).validate({
    ...
    rules: {
      "person[email_attributes][address]": {
        required: true,
        email: true,
        remote: "/people/check_email_availability"
      },
      "person[send_notifications][]": { required: true }
    },
    messages: {
      "person[email_attributes][address]": { remote: email_in_use_message },
      "person[send_notifications][]": {
        required: one_email_must_receive_notifications_message
      }
    },
    submitHandler: function(form) {
...
function initialize_profile_view(profile_id) {
  ...
  $("#load-more-followed-people").on("ajax:complete", function(element, xhr) {
...
function initialize_invitation_form(
  locale,
  email_error_message,
  invitation_limit
) {
...
function initialize_admin_edit_tribe_form(locale, community_id) {
  ...
  translate_validation_messages(locale);
  $("#invite_only_help_text_link").click(function() {
    $("#invite_only_help_text").lightbox_me({ centered: true });
  });
...
function initialize_admin_edit_tribe_look_and_feel_form(
  locale,
  community_id,
  invalid_color_code_message
) {
  translate_validation_messages(locale);
  var form_id = "#edit_community_" + community_id;
  $(form_id).validate({
    rules: {
      ...
      "community[description_color]": {
        required: false,
        minlength: 6,
        maxlength: 6,
        regex: "^([a-fA-F0-9]+)?$"
      },
      "community[slogan_color]": {
        required: false,
        minlength: 6,
        maxlength: 6,
        regex: "^([a-fA-F0-9]+)?$"
...
function initialize_admin_social_media_form(
  locale,
  community_id,
  invalid_twitter_handle_message,
  invalid_facebook_connect_id_message,
  invalid_facebook_connect_secret_message
) {
  translate_validation_messages(locale);
  var form_id = "#edit_community_" + community_id;
  $("#community_facebook_connect_enabled").click(function() {
    $(
      "#community_facebook_connect_id, #community_facebook_connect_secret"
  ...
  $(form_id).validate({
    rules: {
      ...
      "community[facebook_connect_id]": {
        required: false,
        minlength: 1,
        maxlength: 16,
        regex: "^([0-9]+)?$"
      },
      "community[facebook_connect_secret]": {
        required: false,
        minlength: 32,
        maxlength: 32,
        regex: "^([a-f0-9]+)?$"
    ...
    messages: {
      ...
      "community[facebook_connect_id]": {
        regex: invalid_facebook_connect_id_message
      },
      "community[facebook_connect_secret]": {
        regex: invalid_facebook_connect_secret_message
      }
    },
    submitHandler: function(form) {
      disable_and_submit(form_id, form, "false", locale);
...
function initialize_admin_category_form_view(locale, form_id) {
  ...
  var $form = $(form_id);
  var LISTING_SHAPE_CHECKBOX_NAME =
    "category[listing_shapes][][listing_shape_id]";
...
function initialize_pending_consent_form(
  ...
  $("#pending_consent_form").validate({
    ...
    rules: {
      "form[email]": {
        required: true,
        email: true,
        remote: "/community_memberships/check_email_availability_and_validity"
      },
      "form[consent]": { required: true },
      "form[invitation_code]": {
        required: invitation_required,
        remote: "/community_memberships/check_invitation_code"
      }
    },
    messages: {

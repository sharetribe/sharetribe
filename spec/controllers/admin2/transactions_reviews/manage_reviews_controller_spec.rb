...
describe Admin2::TransactionsReviews::ManageReviewsController, type: :controller do
  ...
  describe "#update review" do
    ...
    it 'unskips skipped testimonial' do
      ...
      patch :update_review, params: {id: tx.id, customer_unskip: 1}
      tx.reload
      expect(tx.starter_skipped_feedback).to eq false
      expect(tx.author_skipped_feedback).to eq true
      patch :update_review, params: {id: tx.id, provider_unskip: 1}
      tx.reload
      expect(tx.starter_skipped_feedback).to eq false

describe TransactionService::Process do
  ...
  describe "#get and #create" do
    it "returns with single process" do
      ...
      expect(process.get(community_id: 111, id: id_1).data).to eql({community_id: 111, id: id_1, process: :none, author_is_seller: false})
      expect(process.get(community_id: 222, id: id_2).data).to eql({community_id: 222, id: id_2, process: :preauthorize, author_is_seller: true})
    end
    ...
    it "returns error if specified process not found" do
      ...
      expect(process.get(community_id: 222, id: id_1).success).to eql(false)
      expect(process.get(community_id: 111, id: id_2).success).to eql(false)
    end
    ...
    it "returns with all community processes" do
      ...
      expect(process.get(community_id: 111).data.map(&:attrubutes))

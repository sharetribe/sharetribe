...
RSpec.describe CreateMemberEmailBatchJob, type: :job do
  ...
  let(:person_with_listing_no_payments) { FactoryBot.create(:person, member_of: community) }
  let(:person_no_listing_no_payments) { FactoryBot.create(:person, member_of: community) }
  let(:person_started_transaction) { FactoryBot.create(:person, member_of: community) }

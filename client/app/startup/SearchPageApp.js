...
const getTopbarProps = (topbar, routes) => {
  ...
  const avatarImageRecord = avatarImage
    ? new Image({
        type: ":thumb",
        url: avatarImage
      })
...
export default props => {
  ...
  const bootstrappedData =
    reader.read(_.get(props, "searchPage.data", null)) || Immutable.Map();

  const rawListings = bootstrappedData.get(":data", []);

  const listings = listingsToMap(
    rawListings,
    routes.listing_path,
    routes.edit_listing_path
  );
  const profiles = profilesToMap(
    bootstrappedData.get(":included", []),
    routes.person_path
  );
  ...
  return r(Provider, { store }, [r(SearchPageContainer)]);
};

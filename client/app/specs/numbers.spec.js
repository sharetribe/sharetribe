...
describe("Number utils", () => {
  describe("Formatting distance", () => {
    it('Should format distance as "10,000 mi" with en-US', () => {
      expect(
        formatDistance(new Distance({ value: 10000, unit: ":miles" }), "en-US")
      ).to.equal("10,000 mi");
    });
    it('Should format distance as "10 000 km" with fi-FI', () => {
      expect(
        formatDistance(new Distance({ value: 10000, unit: ":km" }), "fi-FI")
      ).to.equal("10 000 km");
    });
  ...
  describe("Formatting Money", () => {
    it('Should format price as "$ 10,000" with en-US', () => {
      expect(
        formatMoney(
          new Money({ fractionalAmount: 1000000, currency: "USD" }),
          "en-US"
        )
      ).to.equal("$ 10,000.00");
    });
    it('Should format distance as "10 000 €" with fi-FI', () => {
      expect(
        formatMoney(
          new Money({ fractionalAmount: 1000000, currency: "EUR" }),
          "fi-FI"
        )
      ).to.equal("10 000,00 €");
    });
    ...
    it("Should throw error with unknown currency", () => {
      expect(() =>
        formatMoney(
          new Money({ fractionalAmount: 100, currency: "XXX" }),
          "en-US"
        )

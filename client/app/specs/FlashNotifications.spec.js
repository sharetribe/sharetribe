...
describe("FlashNotification", () => {
  describe("actions", () => {
    it("should create an action to add a notification", () => {
      ...
      expect(
        actions.addFlashNotification(type, content).payload.toString()
      ).to.equal(expectedAction.payload.toString());
    });
  ...
  describe("reducer", () => {
    it("should return the initial state", () => {
      ...
      expect(
        Immutable.is(initial.flashMessages, new Immutable.List())
      ).to.equal(true);
    });
    ...
    it("should handle FLASH_NOTIFICATION_ADD", () => {
      ...
      const reducedWithInitialContent = reducer(
        new Immutable.List([flashNote1]),
        {
          type: types.FLASH_NOTIFICATION_ADD,
          payload: flashNote2
        }
      ...
      expect(Immutable.is(reduced, new Immutable.List([flashNote1]))).to.equal(
        true
      );
      expect(Immutable.is(reducedWithInitialContent, expectedList)).to.equal(
        true
      );
    });

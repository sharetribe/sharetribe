...
describe("ManageAvailabilityReducer", () => {
  ...
  const stateTodayBlocked = stateEmpty.set(
    "blocks",
    Immutable.List([Immutable.Map({ id: UUID_V0, day: TODAY })])
  );
  ...
  describe("changes", () => {
    ...
    it("blocks and allows a day", () => {
      const state = applyActions(
        reducer,
        stateEmpty,
        Immutable.List([actions.blockDay(TODAY), actions.unblockDay(TODAY)])
      );
      expect(state.get("blocks").size).to.equal(0);
    ...
    it("allows and blocks again an initially blocked day", () => {
      const state = applyActions(
        reducer,
        stateTodayBlocked,
        Immutable.List([actions.unblockDay(TODAY), actions.blockDay(TODAY)])
      );
  ...
  describe("changes", () => {
    ...
    it("should collect blocks and unblocks", () => {
      ...
      expect(
        isSameDay(block2.get("end"), DAY_AFTER_DAY_AFTER_TOMORROW)
      ).to.equal(true);

...
import {
  blockChanges,
  unblockChanges
} from "../reducers/ManageAvailabilityReducer";
...
const convertBlocksFromUTCMidnightDates = blocks =>
  blocks
    .filter(block => block.getIn([":attributes", ":status"]) !== ":rejected")
    .map(block =>
      block
        .updateIn([":attributes", ":start"], fromMidnightUTCDate)
...
const convertBlocksToUTCMidnightDates = blocks =>
  blocks.map(block =>
    block
      .updateIn(["start"], toMidnightUTCDate)
...
export const changeMonth = day => (dispatch, getState) => {
  dispatch(changeVisibleMonth(day));

  const state = getState().manageAvailability;

  const { start, end } = monthsToLoad(
    day,
    state.get("loadedMonths"),
    PRELOAD_MONTHS
  );
    ...
    harmony
      ...
      .then(response => {
        ...
        const slots = Immutable.Map({
          blocks: convertBlocksFromUTCMidnightDates(
            groups.get(":block", Immutable.List())
          ),
          bookings: convertBlocksFromUTCMidnightDates(
            groups.get(":booking", Immutable.List())
          )
        });
      ...
      .catch(() => {
        ...
        dispatch(
          addFlashNotification(
            "error",
            t("web.listings.errors.availability.something_went_wrong")
          )
        );
      });
  }
  ...
  Promise.all(requests)
    ...
    .catch(e => {
      dispatch(
        addFlashNotification(
          "error",
          t("web.listings.errors.availability.saving_failed")
        )
      );

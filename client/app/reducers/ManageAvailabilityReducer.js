...
const includesDay = (days, day) => days.some(d => isSameDay(d, day));
...
const withChange = (state, action, day) => {
  ...
  if (action === ACTION_UNBLOCK) {
    const currentBlock = state
      .get("blocks")
      .find(b => isSameDay(b.get("day"), day));
...
const mergeNovelty = (state, novelty) => {
  const blocks = novelty.get("blocks").map(b =>
    Immutable.Map({
      id: b.get(":id"),
  ...
  return state
    .set("bookings", state.get("bookings").concat(bookings))
...
export const compressedChanges = state => {
  ...
  const isSameDayChange = c1 => c2 => isSameDay(c1.get("day"), c2.get("day"));
  ...
  const compressed = changes.reduce((result, change) => {
    ...
    if (
      existingChange &&
      existingChange.get("action") !== change.get("action")
    ) {
...
export const hasChanges = state => compressedChanges(state).size > 0;
...
export const blockedDays = state => {
  ...
  const splitChanges = changes.reduce(
    (result, c) => {
      const isBlock = c.get("action") === ACTION_BLOCK;
      const day = c.get("day");
      return isBlock
...
const clearState = state =>
  state
    ...
    .set("visibleMonth", moment().startOf("month"));
  ...
  switch (type) {
    ...
    case actionTypes.UNBLOCK_DAY:
      return saveInProgress
        ? state
        : withChange(state, ACTION_UNBLOCK, payload);

...
const ListingCardBasic = r(ListingCard, {
  className: css.listing,
  color: "#347F9D",
  listing: new ListingModel(basicListingData)
});

const ListingCardNoImage = r(ListingCard, {
  className: css.listing,
  color: "#347F9D",
  listing: new ListingModel(
    Object.assign({}, basicListingData, {
      images: new Immutable.List(),
      price: new Immutable.Map({
        ":money": new Money({
...
const ListingCardImageError = r(ListingCard, {
  className: css.listing,
  color: "#347F9D",
  listing: new ListingModel(
    Object.assign({}, basicListingData, {
...
const ListingCardNoPrice = r(ListingCard, {
  className: css.listing,
  color: "#347F9D",
  listing: new ListingModel(
    Object.assign({}, basicListingData, {
...
const testPrice = function testPrice(card, mountedCard) {
  it("Should display formatted price", () => {
    expect(mountedCard.text()).to.include(
      formatMoney(
        card.props.listing.price.get(":money"),
        card.props.listing.price.get(":priceUnit")
      )
    );
    expect(mountedCard.find(".ListingCard_price")).to.have.length(1);
  });
  it("Should not display order type", () => {
    expect(mountedCard.text()).to.not.include(
      card.props.listing.orderType.get("en")
    );
...
const testDistance = function testDistance(card, mountedCard) {
  it("Should display formatted distance", () => {
    expect(mountedCard.text()).to.include(
      formatDistance(card.props.listing.distance)
    );
  });
};
...
storiesOf("Search results")
  ...
  .add("ListingCard - image fail", () => {
    ...
    specs(() =>
      describe("ListingCard - image fail", () => {
        it('Should display "No picture"', () => {

...
class MenuPriority extends Component {
  ...
  handleResize() {
    ...
    const menuIsVisible =
      window
        .getComputedStyle(priorityMenu, null)
        .getPropertyValue("display") !== "none" &&
      window
        .getComputedStyle(priorityMenu.parentNode, null)
        .getPropertyValue("display") !== "none";
  ...
  updateWidths(links) {
    const linksFromRenderedDiv = Array.prototype.slice.call(
      this.priorityLinksMounted.childNodes
    );
    const withWidths = linksFromRenderedDiv.map(l => {
      const linkData = _.find(
        links,
        link =>
          l.dataset && l.dataset.pid === `${link.content} ${link.priority}`
      );
  ...
  updateNav(linksParam) {
    ...
    const menuButtonWidth =
      menuButtonDOMNode != null ? menuButtonDOMNode.offsetWidth : 0;
    const availableSpace =
      this.menuPriorityMounted.offsetWidth -
      menuButtonWidth -
      EXTRA_SPACING_RIGHT;
    ...
    for (i = 0; i < links.length; i++) {
      if (
        links[i].breakPoint > availableSpace ||
        (this.props.limitPriorityLinks != null &&
          this.props.limitPriorityLinks >= 0 &&
          i >= this.props.limitPriorityLinks)
      ) {
        const noPriorityLinks = this.links.filter(l => l.priority < 0);
        const breakPoint =
          i > 0 ? links[i - 1].breakPoint + ROUNDING_ERROR_MARGIN : 0;
        this.setState({
    ...
    if (
      ...
      links[links.length - 1].breakPoint < availableSpace
    ) {
      const breakPoint =
        links[links.length - 1].breakPoint + ROUNDING_ERROR_MARGIN;
      this.setState({
        // eslint-disable-line react/no-set-state
        priorityWrapperWidth: `${breakPoint}px`,
        priorityLinks: links,
  ...
  render() {
    ...
    const isMeasured =
      this.state.priorityLinks.length > 0 &&
      this.state.priorityLinks[0].breakPoint != null;
    ...
    const fallbackLabel = useFallback
      ? {
          name: this.props.nameFallback,
          menuLabelType: this.props.menuLabelTypeFallback
        }
      : {};
    ...
    const menuProps = Object.assign(
      Object.assign({}, this.props, extraMenuProps)
    );

    return div(
      {
        className: classNames("MenuPriority", css.menuPriority, {
          [css.isMeasured]: isMeasured,
          [css.noPriorityLinks]: useFallback
        }),
        ref: c => {
      ...
      [
        ...
        this.state.hiddenLinks.length > 0 ? r(Menu, menuProps) : null
      ]
    );
  }
}

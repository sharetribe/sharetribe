...
class ListingWorkingHours extends Component {
  ...
  componentWillUpdate(nextProps) {
    ...
    const containsHash =
      window.location.hash.indexOf(
        `#${ListingWorkingHoursActions.EDIT_VIEW_OPEN_HASH}`
      ) >= 0;
    ...
    if (nextProps.isOpen && !containsHash) {
      ...
      setPushState(
        null,
        "Listing working hours is closed",
  ...
  componentWillUnmount() {
    ...
    if (this.props.availability_link) {
      this.props.availability_link.removeEventListener(
        "click",
        this.clickHandler
      );
    }
  ...
  render() {
    ...
    const maybeRenderDefaultLink = this.props.availability_link
      ? null
      : defaultLink;
    ...
    const winder = {
      ...
      onClose: () => {
        ...
        const question = t(
          "web.listings.confirm_discarding_unsaved_availability_changes_question"
        );
...
const mapDispatchToProps = dispatch => ({
  actions: bindActionCreators(
    { ...FlashNotificationActions, ...ListingWorkingHoursActions },
    dispatch
  )
});

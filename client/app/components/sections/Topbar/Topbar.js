...
const mobileProfileLinks = function mobileProfileLinks(
  username,
  isAdmin,
  router,
  location,
  ...
  if (username) {
        ...
        ? [
            r(
              NotificationBadge,
              {
                ...
                countClassName: css.notificationBadgeCount
              },
              unReadMessagesCount
            )
          ]
    ...
    const formatLinkData = function getLink(
      link,
      currentLocation,
      color,
      content
    ...
    if (isAdmin) {
      links.unshift(
        formatLinkData(
          profilePaths.adminDashboardAction,
          location,
          customColor,
...
const isValidSearchParam = value =>
...
class Topbar extends Component {
  render() {
    const { location, marketplace_color1: marketplaceColor1 } = {
      ...DEFAULT_CONTEXT,
    ...
    const newListingRoute =
      this.props.routes && this.props.routes.new_listing_path
        ? this.props.routes.new_listing_path()
        : "#";
    const loginRoute = this.props.routes.login_path
      ? this.props.routes.login_path()
      : "#";
    const signupRoute = this.props.routes.sign_up_path
      ? this.props.routes.sign_up_path()
      : "#";

    // language menu props
    const availableLocales = this.props.locales
      ? this.props.locales.available_locales
      : null;
    ...
    const languageLinks = hasMultipleLanguages
      ? availableLocales.map(locale => ({
          active:
            locale.locale_ident === this.props.locales.current_locale_ident,
    ...
    // mobile menu props
    const profileRoute =
      this.props.routes && this.props.routes.person_path && loggedInUsername
        ? this.props.routes.person_path(loggedInUsername)
        : null;
    const mobileMenuAvatarProps =
    ...
    const searchPlaceholder = this.props.search
      ? this.props.search.search_placeholder
      : null;
    const textLogo = this.props.logo.image ? "" : css.textLogo;

    return div({ className: classNames("Topbar", css.topbar) }, [
      hasMenuProps
        ? r(MenuMobile, { ...mobileMenuProps, className: css.topbarMobileMenu })
        : null,
      r(Logo, {
        ...this.props.logo,
        className: classNames(css.topbarLogo, textLogo),
        color: marketplaceColor1
      }),
      ...
      this.props.search
        ? r(SearchBar, {
            ...
            keywordPlaceholder:
              searchPlaceholder || t("web.topbar.search_placeholder"),
            locationPlaceholder:
              searchPlaceholder == null ||
              this.props.search.mode === "keyword_and_location"
                ? t("web.topbar.search_location_placeholder")
                : searchPlaceholder,
      ...
      div(
        { className: css.topbarMenuSpacer },
        hasMenuProps ? r(MenuPriority, menuProps) : null
      ),

...
const listingsByIds = (listings, ids) => ids.map(id => listings.get(id));

class SearchPage extends Component {
  constructor(props, context) {
    super(props, context);
    this.listings =
      listingsByIds(props.searchPage.listings, props.searchPage.currentPage) ||
      new Immutable.List();
    this.listingProps = this.listingProps.bind(this);

    this.totalPages = Math.ceil(
      this.props.searchPage.state.get("total") /
        this.props.searchPage.state.get("pageSize")
    );
  }
  ...
  listingProps(listing, color, loggedInUsername) {
    ...
    return {
      ...
      loggedInUserIsAuthor:
        loggedInUsername === listing.getIn(["author", "username"])
    };
  }

  render() {
    const {
      marketplace_color1: marketplaceColor1,
      displayBrandingInfo,
      linkToSharetribe
    } = { ...DEFAULT_CONTEXT, ...this.props.marketplace };
    const displayBranding =
      this.props.marketplace && displayBrandingInfo && linkToSharetribe;
          ...
          this.listings.map(listing =>
            r(
              ...
              this.listingProps(
                listing,
                marketplaceColor1,
                this.props.user.loggedInUsername
              )
            )
          )
        )

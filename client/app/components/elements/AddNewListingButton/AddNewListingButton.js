...
export default function AddNewListingButton({
  text,
  url,
  customColor,
  className,
  ...
  return a(
    {
      className: classNames(className, "AddNewListingButton", css.button, {
        [css.responsiveLayout]: !mobileLayoutOnly
      }),
      href: url,
      title: text,
      onMouseOver: e => {
        if (!hasCSSFilters()) {
          ...
          )[0].style.backgroundColor = hoverColor; // eslint-disable-line no-param-reassign
        }
      },
      onMouseOut: e => {
        if (!hasCSSFilters()) {
          e.currentTarget.getElementsByClassName(
            "AddNewListingButton_background"
          )[0].style.backgroundColor = color; // eslint-disable-line no-param-reassign
        }
      }
    },
    [
